...
const loadarticle=async(id)=>{
  try {
    const res=await axios.get(`https://backend-laravelg5.vercel.app/api/api/articles/${id}`)
    setArticle(res.data)
  } catch (error) {
    console.log(error)
  }
    ...
    const handleUpdate=async(e)=>{
      e.preventDefault()
        try {
            await axios.put(`https://backend-laravelg5.vercel.app/api/api/articles/${id}`,article)
            .then(res=>{
                navigate("/articles")
            })
  ...
  return (
    ...
    <button className='btn btn-success btn-sm' onClick={(e)=>handleUpdate(e)}><i className="fa-solid fa-floppy-disk"></i> Enregistrer</button>
    &nbsp;
